...
\begin{abstract}
Mechanistic interpretability labels causal findings with cognitive-science vocabulary without testing whether the evidence supports the specificity those labels imply. We apply 35 validity criteria to a ``lookback mechanism'' reported to track characters' beliefs in Llama-3.1-70B-Instruct through pointer dereference via attention, and identify six untested assumptions---most critically, the evaluation never tests false beliefs: the story-generation code sets belief equal to reality for every pair. We run 15 pre-registered experiments targeting these gaps. The subspaces beat shuffled-label and random-subspace baselines (all $p < 0.05$) and are absent on non-belief tasks (L52/L53 IIA $= 0.0$ on factual recall). Contrary to our pre-registered prediction, they generalize to genuine false-belief stories (L38/L52 IIA $= 0.975$, 100\% behavioral accuracy) and observability-mediated knowledge (L38 IIA $= 0.988$). They are framing-sensitive: belief questions produce IIA $= 1.0$, action-recall questions yield partial transfer (L38 $= 0.886$), reality-state or fill-completion questions produce IIA $= 0.0$. The subspaces encode character-perspective information, not generic entity-state lookup. E-value analysis finds 19 of 20 positive effects robust to unmeasured confounding ($E \geq 2.0$). We upgrade the claim from ``Causally Suggestive'' to ``Mechanistically Supported'' and propose the term \emph{belief-constitutive entity binding}.
\end{abstract}
