...
\begin{abstract}
Prakash et al.\ (ICLR 2026) claim that language models track characters' beliefs via a ``lookback mechanism''---pointer dereference through attention---comprising three sub-mechanisms at non-overlapping layer ranges. We audit this claim using three frameworks: Mechanistic Validity (31 criteria across five validity types), Mechanistic Views (ontological diagnosis of the evidence level), and Mechanistic Reference (referential analysis of the mechanism terms). The audit identifies five structural gaps: no random subspace baseline, no negative controls on non-belief tasks, selection on the dependent variable, untested mechanism unity, and claim laundering---role-level evidence transported to the structural level via vocabulary that imports computational specificity the method cannot detect. The paper's strongest result---a decomposed intervention producing a third answer distinct from both source and counterfactual---demonstrates genuine internal structure, though it remains consistent with generic retrieval. The cognitive science literature on human belief tracking, including unresolved debates over single- versus dual-system accounts and failed replications of automatic belief attribution, provides independent grounds against single-mechanism framing. We pre-register five experiments targeting the gaps. \textcolor{red}{[Results pending.]}
\end{abstract}
...
In the ontology of \citet{tower2026mechviews}, the paper's evidence occupies a single \emph{view}: the role view, where mechanisms are functional roles validated by intervention success. The subspace identification (DCM) parameterizes the search in subspace-view terms---selecting directions on the Grassmannian $\text{Gr}(k, d)$---but validates only by IIA, which is a role-view criterion (does the interchange produce the predicted output?). Confirming a genuine subspace-view claim requires convergent evidence from weight-space analysis: which attention heads implement the retrieval, whether the identified subspace is stable across random seeds, and whether it occupies a consistent location on the Grassmannian across models. The paper provides none of these.

Eleven experiments across three models and dozens of layer-concept combinations appear to constitute massive convergent evidence. They are one measurement family applied repeatedly. Quantity of replications within a view does not substitute for evidence from an independent view.
...
\citet{tower2026mechref} observes that a method achieving high scores on every system, including random systems, has zero referential content. \citet{sutter2025vacuous} demonstrated this for unconstrained DAS: nonlinear alignment maps achieve 100\% IIA on randomly initialized models. DCM constrains the search to binary masks over a fixed SVD basis rather than arbitrary rotations, so it is more constrained than unconstrained DAS. Whether these constraints are sufficient to avoid vacuousness is an empirical question: if DCM finds low-rank subspaces with high IIA on a randomly initialized model, the method inherits the Sutter problem despite its constraints. We include this as a control in Experiment~2.
...
\paragraph{Claim laundering (V1, V4).}
...
\citet{tower2026mechref} identifies this pattern as \emph{claim laundering}: role-level evidence (``swapping vectors at certain layers changes the output'') is transported to the structural level (``LMs implement a pointer dereference mechanism'') via vocabulary that imports computational specificity the method cannot detect. The evidence establishes that information flows through layer ranges in ways consistent with a retrieval process. The labels ``pointer,'' ``address,'' and ``dereference'' carry the inferential weight from role to structure---strip the metaphor and the two-level leap becomes visible.
...
Applying the reference-narrowing procedure from \citet{tower2026mechref}---contracting the claim to the transport level the evidence licenses---yields the following restated claim:
