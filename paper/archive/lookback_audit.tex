...
\begin{abstract}
Mechanistic interpretability routinely labels causal findings with vocabulary borrowed from computer science and cognitive science---pointers, dereferencing, theory of mind---without testing whether the evidence supports the specificity these labels imply. We apply 31 validity criteria across five types (construct, measurement, internal, external, interpretive) to a recent case: a three-part ``lookback mechanism,'' in which language models are reported to track characters' beliefs through pointer dereference implemented via attention, with each sub-mechanism localized to a non-overlapping layer range. The audit identifies six untested assumptions: no random subspace baseline, no negative controls on non-belief tasks, selection on the dependent variable, untested mechanism unity, no test of compositional generalization, and referential inflation---vocabulary that imports computational specificity the method cannot detect. The strongest result---a decomposed intervention producing a third answer distinct from both source and counterfactual---demonstrates genuine internal structure, though it remains consistent with generic retrieval rather than belief-specific processing. Independent evidence from cognitive science, including unresolved single- versus dual-system debates and failed replications of automatic belief attribution in humans, weighs against the single-mechanism framing. We pre-register six experiments targeting these gaps. \textcolor{red}{[Results pending.]}
\end{abstract}
...
We pre-register six experiments targeting the gaps identified above, organized as three primary experiments (run first) and three secondary experiments (run if primary experiments succeed). All experiments use Llama-3-70B-Instruct, reproducing the original paper's setup via NNsight. All $p$-values across the six experiments are Benjamini-Hochberg corrected for the total number of pre-registered tests before interpreting significance. \textcolor{red}{[Pre-registration SHA and results to be added.]}
...
The six experiments target gaps that are testable and whose outcomes change the paper's verdict in either direction. If the identified subspaces generalize to a disjoint SVD basis, beat shuffled-label controls, and are absent on non-ToM tasks, the claim advances from Causally Suggestive to Mechanistically Supported. That would be the most useful outcome: the audit would confirm the result by providing the missing controls rather than undermining it.

The held-out basis validation (Experiment~1) is the most consequential experiment. The binary mask is already validated out-of-sample (trained on 80 stories, evaluated on a separate 80). What remains untested is whether the SVD basis---the candidate directions the mask selects from---generalizes to a disjoint story pool. If it does, the feature-space leakage concern closes. The matched-capacity controls (Experiment~2) then test belief-specificity: do real-label subspaces outperform shuffled-label subspaces trained with identical optimization budgets? Given the existing out-of-sample mask validation, this comparison is likely to favor the real-label subspace, and we pre-commit to reporting M2 as satisfied if it does.

All claims in the paper rest on a single method family (interchange interventions via NNsight), producing evidence of one kind \citep{tower2026mechviews}. The experiments in Section~\ref{sec:experiments} add independent views: held-out basis validation tests whether the subspace-view claim survives out-of-sample evaluation, task-specificity controls test discriminant validity at the role level, and cross-stage mediation probes causal dependencies invisible to single-site intervention.

An interchange intervention on the full residual stream transfers all information at a token position. When the claim targets a specific substructure, the instrument and the claim are misaligned---more data from the same instrument cannot close this gap. The task-specificity controls (Experiment~3) address this by testing whether the subspace transfers to non-belief tasks, isolating the interpretive question: is this a belief-tracking mechanism or an entity-state retrieval mechanism? The distinction is substantive: entity-state retrieval corrects the label rather than refuting the finding.

\paragraph{Contracted claim.}
Contracting the claim to what the evidence licenses \citep{tower2026mechref} yields the following restated claim:

\begin{quote}
In Llama-3-70B-Instruct, on pre-filtered correct-answer synthetic stories with fixed template structure, interchange interventions on residual stream vectors at state tokens (layers 33--38) and at the final token (layers 52--56) alter belief-tracking outputs. A third intervention site (visibility, layers 10--31) operates when explicit visibility conditions are present. Qualitatively similar patterns appear in two other models at different layer ranges and subspace dimensions.
\end{quote}
...
These gaps are common in mechanistic interpretability. We chose the Lookback paper because it is well-executed within its scope, published at a top venue, and representative of current practice---making it a useful case study for what structured validity assessment adds to the standard publication workflow.

\paragraph{Limitations.}
This audit applies validity criteria developed by the same author. The framework is independently deposited with full criterion definitions and pass conditions, so readers can verify whether our ratings match the stated standards. The six pre-registered experiments provide a less subjective complement: their outcomes are determined by data, not by the auditor's assessment. \textcolor{red}{[To be updated after experiments.]}
