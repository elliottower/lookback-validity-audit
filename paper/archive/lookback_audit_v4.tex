...
\begin{abstract}
Mechanistic interpretability routinely labels causal findings with vocabulary borrowed from cognitive science without testing whether the evidence supports the specificity these labels imply. We apply 35 validity criteria across five types to a three-part ``lookback mechanism'' reported to track characters' beliefs in Llama-3.1-70B-Instruct through pointer dereference implemented via attention. The audit identifies six untested assumptions, most critically that the evaluation never tests false beliefs: the story-generation code sets belief equal to reality for every evaluated pair. We conduct 15 pre-registered experiments targeting these gaps. The identified subspaces beat shuffled-label and random-subspace baselines (all $p < 0.05$) and are absent on non-belief tasks (L52/L53 IIA $= 0.0$ on factual recall), confirming they capture genuine, task-relevant structure. The decisive experiments concern belief specificity: contrary to our pre-registered prediction, the subspaces generalize to genuine false-belief stories (L38/L52 IIA $= 0.975$, 100\% behavioral accuracy) and to observability-mediated knowledge (L38 IIA $= 0.988$). The subspaces are framing-sensitive: belief questions produce IIA $= 1.0$, action-recall questions yield partial transfer (L38 $= 0.886$), and reality-state or fill-completion questions produce IIA $= 0.0$. The subspaces encode character-perspective information, not generic entity-state lookup. A confounding sensitivity analysis finds 19 of 20 positive effects robust to unmeasured confounding (E-value $\geq 2.0$). We upgrade the claim from ``Causally Suggestive'' to ``Mechanistically Supported'' and propose the term \emph{belief-constitutive entity binding} for the mechanism's functional role.
\end{abstract}
...
Mechanistic interpretability attributes causal roles to model components using interchange interventions---swapping internal representations between paired inputs \citep{geiger2021causal, vig2020investigating}. When such experiments succeed, the result is typically described as ``the mechanism'' for a capability: induction heads for in-context learning \citep{olsson2022context}, the IOI circuit \citep{wang2023interpretability}, the ``lookback mechanism'' for belief tracking \citep{prakash2025lookback}. These labels carry validity assumptions---construct, measurement, internal, external, and interpretive---that are rarely tested.

We audit \citet{prakash2025lookback}, who claim that large language models implement belief tracking via three ``lookback'' sub-mechanisms---binding (layers 33--38), answer (layers 52--56), and visibility (layers 10--31)---collectively described as ``pointer dereference via attention.'' We apply 35 validity criteria across five types (construct, measurement, internal, external, interpretive) and identify six untested assumptions.\footnote{We draw on three deposited frameworks: Mechanistic Validity \citep{tower2026mechval}, which provides the 35 criteria; Mechanistic Views \citep{tower2026mechviews}, which asks what \emph{kind} of claim a given method can support; and Mechanistic Reference \citep{tower2026mechref}, which asks whether a mechanism's name refers to anything the method can actually detect. Each is independently deposited; the audit's findings stand without them.} The claim rests on interchange intervention experiments on 80 synthetic stories in Llama-3-70B-Instruct, pre-filtered to cases the model answers correctly, with qualitative replication in two other models. The three sub-mechanisms operate on different token types at different layer ranges, with per-layer subspace ranks varying from 3 to 19 singular vectors for the pointer concept and up to 320 for visibility (selected from a truncated SVD of the 8192-dimensional residual stream).\footnote{Ranks extracted from released results at \url{https://github.com/Nix07/belief_tracking} (commit \texttt{0579347e}); the SVD uses 500 components. See \texttt{reference/extracted\_results\_llama70b.json} in our repository.}

The audit produces a verdict on the five-tier scale (Proposed, Causally Suggestive, Mechanistically Supported, Triangulated, Validated) and pre-registers six experiments to test the gaps. Section~\ref{sec:discussion} narrows the claim to what the evidence licenses.
...
The paper tests no task where the lookback pattern should be absent. Candidate negative controls: factual recall without belief tracking, simple copying, single-character stories, non-ToM multi-step retrieval. If the pattern appears on all of these, it is attention-mediated retrieval, not a belief-tracking mechanism.
...
Eleven experiments across three models and dozens of layer-concept combinations appear to constitute massive convergent evidence. They are one measurement family applied repeatedly. Quantity of replications within one method does not substitute for evidence from an independent method.
...
The subspace identification procedure (SVD followed by L1-regularized binary mask selection) produces per-layer subspaces of varying rank---typically 3 to 19 singular vectors for the pointer concept, selected from a truncated SVD of the 8192-dimensional residual stream. No random subspace of matched rank is tested for comparable IIA. In several reported experiments, the subspace intervention approaches the full residual stream intervention's IIA (e.g., 0.925 vs.\ 1.0 at layer 38 for the pointer concept), suggesting the subspace captures most task-relevant signal while leaving a non-trivial gap. The visibility lookback is particularly striking: the selected ``subspace'' for the source concept uses 160--320 of 500 SVD components (32--64\% of the truncated candidate basis, though only 2--4\% of the full 8192-dimensional residual stream), where a weaker L1 penalty ($\lambda = 0.03$ vs.\ $0.1$) permits the optimizer to retain most candidate dimensions. At early layers, the selected subspace achieves higher IIA than the full 500-component basis (e.g., 0.90 vs.\ 0.175 at layer 7), suggesting that the remaining SVD components destroy counterfactual behavior---a pattern consistent with meaningful directional selectivity or optimization artifacts.
...
We pre-register six experiments targeting the gaps identified above, organized as three primary experiments (run first) and three secondary experiments (run if primary experiments succeed). All experiments use Llama-3-70B-Instruct, reproducing the original paper's setup via NNsight. All $p$-values across the six experiments are Benjamini-Hochberg corrected for the total number of pre-registered tests before interpreting significance. The pre-registration document was committed to the repository at tag \texttt{prereg-amendment-5} before results were computed; subsequent amendments (7--9) are SHA-tagged in the \texttt{AMENDMENTS.md} file.
...
All experiments use Llama-3.1-70B-Instruct via NNsight/NDIF remote execution, applying the paper's released SVD bases and singular-vector masks to compute interchange intervention accuracy (IIA). We report results grouped by validity type.

\subsection{Measurement validity: replication and baselines (E1--E2)}

\paragraph{E1: Positive control replication.}
We replicate the paper's interchange interventions on Llama-3.1-70B-Instruct (the paper uses Llama-3.0-70B-Instruct). The primary binding subspace (L34) replicates within $\pm 0.05$ (IIA $= 0.988$ vs.\ paper's $0.963$). The answer subspaces show \emph{higher} IIA than reported: L38 achieves $1.0$ (paper: $0.925$), L52 achieves $1.0$ (paper: $0.775$), and L53 achieves $1.0$ (paper: $0.550$). The L36 binding subspace drops substantially ($0.225$ vs.\ paper's $0.750$). The upward shifts at L52 and L53 suggest the paper's single-seed methodology (seed 10) underestimates these subspaces' typical IIA; multi-seed replication in a previous amendment confirmed L53 variance spanning $0.55$--$0.99$ across seeds (Figure~\ref{fig:replication}).
...
All six subspaces beat random subspaces of matched rank ($p = 0.005$, rank test with $n = 200$ random draws). Against shuffled-label controls ($n = 20$ permutations each), four subspaces pass at $p < 0.05$: L34 binding ($p = 0.010$), L52 answer ($p = 0.010$), L38 answer ($p = 0.048$), and L35/L36 binding ($p = 0.048$ each). L53 does not reach significance against shuffled labels ($p = 0.286$), consistent with its high seed-to-seed variance. The real-label IIA values ($0.55$--$0.96$) are well separated from shuffled-label means ($0.47$--$0.50$), confirming the subspaces capture task-relevant structure beyond what chance optimization produces.

\subsection{Construct validity: specificity and generalization (E10--E11)}

\paragraph{E10: Cross-template generalization.}
The answer subspaces trained on template 2 (the paper's default) transfer to template 0 stories ($n = 23$): L38 IIA $= 0.957$, L52/L53 IIA $= 0.783$. The subspaces are not template-specific.
...
These experiments, pre-registered in Amendment~9, test whether the subspaces encode generic entity-state bindings or character-perspective-specific information.

\paragraph{E12: Question-framing control ($n = 35$ shared pairs).}
We generate clean/counterfactual pairs using the CausalToM template, then reframe the question in four ways while preserving the story and answer: \emph{belief} (``What does X believe the container contains?''), \emph{action recall} (``What did X put in the container?''), \emph{reality state} (``What is inside the container?''), and \emph{fill completion} (``X filled the container with'').

Results in Table~\ref{tab:framing}. The subspaces are framing-\emph{sensitive}: belief framing produces perfect IIA ($1.0$ across all subspaces), action recall yields partial transfer (L38 $= 0.886$, L52 $= 0.400$, L53 $= 0.057$), and reality-state and fill-completion framings produce IIA $= 0.0$ across all subspaces. This \emph{overturns} our pre-registered prediction that all framings would be within $0.10$ of the belief baseline.

The pre-committed interpretation for this outcome is: ``the subspace may be sensitive to epistemic question framing.'' The result distinguishes two hypotheses. If the subspaces encoded generic entity-state bindings (``what drink is associated with what container''), reality-state and fill-completion questions---which request the same underlying fact---should activate the same subspace. They do not. The subspaces specifically encode information relevant to character-perspective queries.
...
We insert irrelevant drink mentions at three positions: early (after scene-setting), mid (between fill actions), and late (before the question). On the unmodified baseline, L38/L52 IIA $= 1.0$, L53 $= 0.7$. Mid- and late-position distractors destroy behavioral accuracy entirely (zero pairs survive model-accuracy filtering). Early distractors retain 21 pairs, but IIA drops to $0.0$ across all subspaces. The subspace is positionally fragile: even minor template perturbations disrupt the mechanism. This limits the scope of the belief-tracking claim to the specific template structure used in training, though it does not bear on whether the mechanism encodes belief vs.\ generic retrieval.

\paragraph{E14: True false-belief test ($n = 80$, 100\% behavioral accuracy).}
We construct stories in which a character places a drink, a second character swaps it without the first character's knowledge, and the question asks what the first character \emph{believes} the container contains. The model answers all 80 false-belief stories correctly (reporting the original drink, not the swapped one), confirming it can perform false-belief reasoning.

The subspaces achieve IIA $= 0.975$ at both L38 and L52 (95\% Wilson CI $[0.913, 0.993]$; L53 $= 0.388$). This \emph{overturns} our pre-registered prediction of IIA $< 0.30$. The pre-committed interpretation: ``the subspace genuinely tracks the character's epistemic state even when it diverges from reality\ldots the strongest possible evidence for the paper's belief tracking interpretation.'' The subspaces trained on stories where belief equals reality generalize to stories where belief and reality diverge.

The reality-question control (asking what is \emph{actually} in the container) yielded zero viable pairs: the model could not reliably answer reality-state questions about false-belief stories. This control is therefore uninformative.

\paragraph{E15: Observability dissociation ($n = 80$, 88\% behavioral accuracy).}
Template 1 creates an observability asymmetry: character 1 can observe character 2's actions, but not vice versa. We ask character 1 about the container that character 2 filled (observed-other condition). The model answers correctly for 212 of 240 generated pairs (88\%); we evaluate the first 80 passing pairs.

L38 achieves IIA $= 0.988$ (95\% Wilson CI $[0.932, 0.998]$) and L52 achieves $0.975$ $[0.913, 0.993]$ (L53 $= 0.213$). This again \emph{overturns} our prediction of IIA $< 0.30$. The subspaces generalize to knowledge acquired through observation, not just through the character's own actions.

The self-container control (character 1 asked about their own container using template 1) yielded zero viable pairs---the model could not answer self-container queries on this template. This control is uninformative.

\subsection{Experiments with incomplete data}

Several pre-registered experiments yielded incomplete or underpowered results due to NDIF server instability (WebSocket crashes, write timeouts) or model-accuracy filtering:

\begin{itemize}
\item \textbf{E5} (compositional generalization): most conditions yielded zero pairs surviving model-accuracy filtering (3-character scaling, nested beliefs, content types other than existence). The model does not reliably handle these structural variations, making the compositional generalization question untestable with the current approach.
\item \textbf{E6} (cross-stage mediation): only synthetic (dry-run) results obtained; NDIF instability prevented real computation.
\item \textbf{E7--E9} (necessity, layer spread, rank decomposition): real IIA computation not completed due to repeated NDIF session crashes.
\item \textbf{E4} (surface heuristic): only dry-run results obtained.
\end{itemize}

These incomplete experiments do not affect the paper's central conclusions, which rest on E1--E2 (measurement validity), E10--E11 (specificity), and E12--E15 (the belief-vs-binding dissociation battery).

\subsection{Confounding sensitivity (I8, exploratory)}

To assess whether the observed IIA effects could be explained by unmeasured confounders, we compute E-values \citep{vanderweele2017sensitivity} for all 20 positive effects (IIA $>$ baseline) across experiments. The E-value quantifies how strong an unmeasured confounder would need to be to explain away the observed effect: $E = \text{RR} + \sqrt{\text{RR} \cdot (\text{RR} - 1)}$, where $\text{RR} = \text{IIA} / \text{baseline}$ and baseline IIA $= 0.5$ (chance under random subspace swap).

Of 20 positive effects, 19 achieve $E \geq 2.0$ (Figure~\ref{fig:evalues}). The single fragile effect is L38 on factual recall ($E = 1.31$, IIA $= 0.529$), consistent with L38's partial cross-task transfer identified in E11. The strongest effects (replication, false belief, observability, belief framing) achieve $E = 3.31$--$3.41$. This analysis is labeled exploratory because it was added after initial pre-registration (Amendment~10c).
...
The experiments produce a verdict we did not predict. We entered the audit expecting the subspaces to encode generic entity-state bindings that happen to align with beliefs on the paper's task. Three of our four dissociation experiments overturned their pre-registered predictions, each time in the direction that \emph{strengthens} the paper's claim.

The false-belief test (E14) is the strongest single result. The paper's CausalToM dataset never tests false beliefs: \texttt{Dataset.set\_story()} uses only \texttt{template["context"]}, excluding the \texttt{causal\_event} field, and always queries characters about their own containers (\texttt{set\_character=rc, set\_container=rc}), so belief equals reality for every pair. We constructed 80 stories with genuine drink swaps that the queried character does not observe. The model answers all 80 correctly (reporting the believed drink, not the actual one), and the subspaces achieve IIA $= 0.975$ at L38 and L52. The subspace trained on stories where belief and reality coincide generalizes to stories where they diverge---evidence the paper never collected but that supports its claim.

The observability test (E15) extends this finding. We ask a character about a container filled by a different character, where the queried character observed the filling. The subspaces achieve IIA $= 0.988$ (L38) and $0.975$ (L52). The mechanism generalizes to knowledge acquired through observation, not just through the character's own actions.

The framing result (E12) is the most informative for understanding what the subspaces encode. We predicted framing-invariance: that belief, action-recall, reality-state, and fill-completion questions would produce matched IIA, indicating a generic entity-state lookup. The opposite occurred. The subspaces specifically activate for character-perspective queries (belief IIA $= 1.0$), show graded transfer to action recall (L38 $= 0.886$, L52 $= 0.400$), and are silent for perspective-free queries (reality-state and fill-completion IIA $= 0.0$). The underlying entity-state fact---which drink is in which container---is identical across all four framings. The subspace responds to the framing, not the fact.

This pattern resolves the audit's central question. The subspaces do not encode ``what entity is associated with what state'' (which would predict framing-invariance). They encode information accessible through character-perspective queries---information that updates when characters observe events (E15) and that tracks beliefs diverging from reality (E14). The referential inflation concern raised in Section~\ref{sec:validity} was warranted as an a priori worry, but the experiments show the mechanism is more specific than generic retrieval.
...
The distractor result (E13) reveals positional fragility: even minor template perturbations destroy behavioral accuracy (mid/late: zero surviving pairs) or subspace function (early: IIA $= 0.0$). The belief-tracking claim holds within the template structure used for training, but the mechanism may not survive naturalistic text variation. Several pre-registered experiments (E4--E9) could not be completed due to NDIF server instability and model-accuracy filtering failures; the compositional generalization question (E5) remains open. All experiments use one model (Llama-3.1-70B-Instruct, a minor version update from the paper's Llama-3.0). We use the paper's released SVD bases and singular-vector masks rather than recomputing them, so our results test the \emph{generalization} of these subspaces to novel stimuli rather than their \emph{replicability} from scratch. The self-container and reality-question controls in E15 and E14, respectively, yielded zero viable pairs, leaving those comparisons uninformative. This audit applies validity criteria developed by the same author; the framework is independently deposited with full criterion definitions so readers can verify whether ratings match the stated standards.

\paragraph{Verdict.}
We upgrade the claim from ``Causally Suggestive'' to ``Mechanistically Supported'' (Figure~\ref{fig:heatmap}). The subspaces replicate on a minor model version update (Figure~\ref{fig:replication}), beat shuffled-label and random-subspace baselines (measurement validity), are absent on non-belief tasks at L52/L53 (construct validity), generalize to false beliefs and observability-mediated knowledge (external validity), are framing-sensitive rather than framing-invariant (interpretive validity, Figure~\ref{fig:framing}), and 19 of 20 positive effects are robust to unmeasured confounding (Figure~\ref{fig:evalues}). The remaining gap to ``Triangulated'' would require convergent evidence from an independent method (weight analysis, probing) and cross-model replication with proper multi-seed statistics.

\paragraph{Revised claim.}
The original audit proposed contracting ``belief tracking via pointer dereference'' to ``attention-mediated retrieval.'' The E12--E15 results warrant a more specific characterization. The subspaces encode character-state associations that are updated by observability and state-change processing, and that are accessible specifically through character-perspective queries. This is richer than ``retrieval'' but more precise than ``belief tracking''---it is belief tracking implemented as entity-state binding, not belief tracking as a separate cognitive primitive. We propose the term \emph{belief-constitutive entity binding}: the entity bindings constitute beliefs in the functional sense, encoding what each character would report about each container.

\paragraph{Implications for mechanistic interpretability.}
This case study demonstrates the value of pre-registered validity audits. Three of four dissociation experiments overturned their predictions---each time strengthening rather than weakening the original claim. Without pre-registration, these results could be dismissed as post-hoc rationalization. With pre-registration, the pattern of overturned predictions is itself evidence: we designed experiments to find entity binding and found belief tracking instead. The framing-sensitivity result (E12), which we expected to undermine the paper's interpretation, provides the clearest evidence \emph{for} it.
