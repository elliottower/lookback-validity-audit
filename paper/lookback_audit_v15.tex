...
\begin{abstract}
Causal findings in mechanistic interpretability are described with specific labels (belief tracking, planning, deception) whose specificity outruns what the causal evidence can verify. \citet{prakash2025lookback} report a ``lookback mechanism'' in Llama-3.1-70B-Instruct that tracks characters' beliefs through pointer dereference via attention, identifying low-rank subspaces at three layer ranges that carry belief content when swapped between stories. We apply the 36 criteria of the Mechanistic Validity framework and identify six untested assumptions, most critically that the evaluation never tests false beliefs: the story-generation code sets each character's belief equal to reality in every pair. Six experiments were registered before data collection and nine more in timestamped amendments before their own results were computed; nine of the fifteen completed. All six subspaces beat matched random subspaces; five of six beat shuffled-label controls at $p < 0.05$, with L53 failing to separate ($p = 0.286$) and unstable across seeds. They generalize to genuine false-belief stories (IIA $= 0.975$) and observability-mediated knowledge (IIA $= 0.988$), overturning our pre-registered prediction that they would not. Question framing modulates transfer. At $n = 250$ with per-framing accuracy filtering, belief questions give IIA $= 1.0$ across all three subspaces and action recall $0.98$--$1.0$, while reality-state questions over the same story pairs give $0.916$, $0.82$ and $0.82$. The reduction is reliable and the Wilson intervals do not overlap, but transfer is not abolished: the subspaces are framing-sensitive rather than belief-exclusive. A smaller $n = 35$ analysis without per-framing filtering showed no transfer under reality framing; per-framing filtering removes that contrast, as it also does for action recall, and fill-completion is not yet computed at $n = 250$. A cross-model mismatch test on Qwen2.5-14B-Instruct confirms the answer lookback's dependency structure transfers (singles near zero, pair at $1.0$) while the binding mechanism shows a structurally different pattern---partial transport that full-residual patching cannot detect. We upgrade the answer lookback at L38 and L52 from ``Causally Suggestive'' to ``Mechanistically Supported'' and leave the binding lookback and L53 at ``Causally Suggestive,'' since the carving into three sub-mechanisms is not itself established. We propose the term \emph{perspective-indexed entity binding}.
\end{abstract}
...
groups, offering lexical and reflexive retrieval as alternatives. Their result is the
empirical form of the distinction \citet{vegner2025systematicity} draw: behavioral
...
E13 reveals positional fragility: minor template perturbations destroy behavioral accuracy (mid/late: zero surviving pairs) or subspace function (early: IIA $= 0.0$). The belief-tracking claim holds within the training template structure; the mechanism may not survive naturalistic text. E4--E9 could not be completed due to NDIF server instability and model-accuracy filtering; compositional generalization (E5) is withdrawn rather than deferred. Its stimuli yielded no viable pairs, and the question was answered at larger scale before we re-ran it: \citet{gurarieh2025mixing} evaluate the positional mechanism across nine models and ten tasks and report that it does not generalize beyond settings with two or three entity groups, offering lexical and reflexive retrieval as alternative mechanisms. Those are rival explanations this audit did not consider under I5. All Llama experiments use one model (Llama-3.1-70B-Instruct, a minor version update from the paper's 3.0). X2 extends to Qwen2.5-14B-Instruct, where the answer lookback dependency structure transfers but the binding mechanism operates differently. We use the paper's released SVD bases and masks rather than recomputing them, so our results test \emph{generalization} to novel stimuli rather than \emph{replicability} from scratch. Self-container controls yielded zero viable pairs; the reality-question control yielded 250 pairs under per-framing filtering and is reported in Section~\ref{sec:experiments}. This audit applies validity criteria developed by the same author; the framework is independently deposited so readers can verify ratings against the stated standards.
