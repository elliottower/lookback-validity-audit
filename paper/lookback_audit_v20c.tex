...
EXP19--EXP20 & Amendment 13 & complete & Section~\ref{sec:exp1920} \\
...
0.0196$, and $(0+1)/(100+1) = 0.0099$ at L34. The registration set the bar at $p < 0.01$ against
200 draws; the shortfall in draws and its consequence are recorded under Deviations. Separation is not uniform in rank: at rank 3 every
...
\subsection{What the both-condition outputs (EXP19--EXP20)}
\label{sec:exp1920}

The interaction result above rests on the both-condition scoring zero: patching the recalled
and lookback groups together restores none of the counterfactual answer. Zero is compatible
with three different things happening inside the model, and the coefficient is the same under
all three. The patches could cancel and leave the clean computation intact; they could transfer
the counterfactual answer, which the scoring would then have mismeasured; or they could destroy
the computation and emit something belonging to neither story. EXP19 and EXP20 were registered
in Amendment 13, before either was run, to separate them.

\paragraph{EXP19: full-sequence positive control (gate).}
Patching every position must reproduce the counterfactual answer, or the harness cannot detect
transfer at all and no null from it means anything. The gate passes at both layers: $30/30$
hits at L26 and $30/30$ at L30, with no undefined observation. Amendment 16a moved this gate
onto answer pairs, because on binding pairs the clean and counterfactual answers coincide by
construction and the test would be vacuous.

\paragraph{EXP20: output identity.}
Every readout in the both-condition is classified into one of three exhaustive categories---the
clean answer, the intervention target, or neither---and a conclusion requires one to beat both
others under exact conditional binomial tests, Holm-adjusted across the six comparisons. The
result is unambiguous. At both layers, all $200$ readouts return the clean answer: $200$ clean,
$0$ target, $0$ off-target, giving $p_{\text{Holm}} = 7.5 \times 10^{-60}$ against each
alternative. Clean accuracy is $1.0$ and the both-condition interchange accuracy is $0.0$ with a
Wilson upper bound of $0.019$. Two independent resolvers agree on every observation
($1.0$ agreement, $0$ disagreements), and no observation is undefined across $2{,}800$ scored
readouts.

The verdict is \textbf{cancellation}, not transfer and not interference. Two further quantities
say the patches are nonetheless doing something. The margin between the clean-answer logit and
the intervention-target logit is positive on every pair, with a median of $+18.3$ at L26 and
$+18.7$ at L30 and a minimum of $+1.7$, so the model is not marginally preferring the clean
answer but decisively so. And the combined patch is not a no-op: it reproduces the unpatched
argmax on $200/200$ pairs while reproducing the unpatched top-ten ordering on $0/200$. The
distribution is perturbed and lands back on the same answer.

This is what licenses reading the order-2 coefficient as interaction rather than as a scoring
artifact. It also bounds the reading: cancellation shows the two groups are jointly
inert on the answer, not that either is inert alone, and the singles were already near zero.
...
Two deviations occurred in EXP2. Neither alters the direction of the result, and both concern
how much of the registered design was executed rather than what it returned.

\paragraph{Draw count, and the threshold it puts out of reach.}
The registration specifies 200 random index sets per subspace and a decision rule of $p < 0.01$
on the one-sided rank test $p = (k+1)/(n+1)$. We ran 50 per subspace, extended to 100 at L34.
The rule and the draw count interact: with no random draw reaching the identified value, $p$ is
$1/(n+1)$, so $n = 50$ bottoms out at $0.0196$ and cannot return a value below $0.01$ whatever
the separation. Five of the six subspaces are therefore reported as separating without meeting
the registered threshold, and L34 meets it at $0.0099$. The shortfall is in $n$, fixed before
any intervention ran, rather than in what the draws returned: no random subspace of matched
rank reached the identified value at any layer, and at rank 3 every draw returned exactly
$0.000$ against identified values of $0.925$ and $0.963$.

\paragraph{The coherent-third-answer arm was not run.}
The registration adds a second measurement to EXP2: the rate at which each random subspace
produces a coherent third answer, defined in advance as a token from the story's entity
vocabulary distinct from both the clean and the counterfactual answer, to test whether the
decomposable structure is direction-specific rather than a property of the basis geometry. That
rate is not computed here. The per-intervention records hold each prediction and its target but
not the clean answer or the story vocabulary, so the arm is not recoverable from the stored
observations and would need the evaluation pairs alongside them.

Neither deviation is load-bearing for M2 as the framework defines it, which asks whether the
measurement separates from a matched baseline; the separation is absolute on the draws taken.
Both bound what the experiment can be cited for: an identified subspace that no matched-rank
random draw approaches, on 350 draws, and no evidence either way on whether the third answers
...
cannot disagree. The third is a second case: \citet{cui2026dual} apply the
