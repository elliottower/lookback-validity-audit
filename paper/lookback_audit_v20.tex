...
\begin{abstract}
Causal findings in mechanistic interpretability carry specific labels---belief tracking, planning, deception---whose specificity is rarely tested. \citet{prakash2025lookback} report a ``lookback mechanism'' in Llama-3.1-70B-Instruct that tracks characters' beliefs through pointer dereference via attention, identifying low-rank subspaces at three layer ranges that carry belief content when swapped between stories. We apply the 36 criteria of the Mechanistic Validity framework and identify six untested assumptions, most critically that the story-generation code sets each character's belief equal to reality in every pair, so the evaluation never tested the phenomenon the claim is named for. Fifteen experiments were pre-registered in timestamped amendments; nine completed, three of them overturning our own predictions. (1)~The subspaces transfer to stories where belief diverges from reality (IIA $= 0.975$) and observability-mediated knowledge (IIA $= 0.988$), overturning our pre-registered prediction that they would not. Question framing modulates transfer: belief questions give IIA $= 1.0$ while reality-state questions give $0.82$--$0.92$ over the same story pairs, a reliable reduction that does not abolish transfer. (2)~A cross-model mismatch test on Qwen2.5-14B-Instruct confirms the answer lookback's dependency structure transfers (singles near zero, pair at $1.0$) while the binding mechanism shows a structurally different pattern---partial transport that full-residual patching cannot detect. (3)~That test is a complete two-player coalition function, so the interaction between its token groups is exact: binding is a parity function at layers 24--34, with both order-1 coefficients exactly zero and all non-constant energy at order~2, while the answer lookback is a conjunction at layers 32--38 ($w_2 = +0.249$ against a ceiling of $+0.25$). A summary reporting marginal contributions assigns binding two zeros. All readings stay at ``Causally Suggestive'' or below: the ladder gates baseline separation (M2) beneath every causal rung, and no matched-rank random-subspace comparison exists in the record, so the accumulated evidence does not move the tier. We propose the term \emph{perspective-indexed entity binding}.
\end{abstract}
...
(3)~An exact interaction decomposition of the cross-model test, which finds the two sub-mechanisms carrying pairwise interactions of opposite sign, each saturated at the bound the design admits.
(4)~A verdict per reading rather than one for the claim, with baseline separation identified as the single gate holding every reading at ``Causally Suggestive'' or below, and a revised term: \emph{perspective-indexed entity binding}.
...
\paragraph{No cross-task transfer (EXP3).}
...
\paragraph{No graded response (EXP5).}
...
\paragraph{No novel prediction (EXP6).}
...
\subsection{Measurement validity: replication (EXP1)}
...
\begin{table}[t]
\centering
\caption{EXP12: IIA by question framing ($n = 35$ shared pairs) with 95\% Wilson confidence intervals. The subspaces specifically respond to belief framing, partially to action recall, and not at all to perspective-free queries about reality or fill completion. See Figure~\ref{fig:framing}.}
\label{tab:framing}
\begin{tabular}{lccc}
\toprule
...
\subsection{Cross-model dependency structure (EXP16)}
...
\subsection{Interaction structure of the mismatch test (EXP16, reanalysis)}
...
as independent binomials; the three patched conditions share the same pairs, so the paired
interval is narrower than the one reported.}
...
The two mechanisms take interactions of opposite sign, each saturated (Table~\ref{tab:interaction},
Figure~\ref{fig:interaction}). From layer 24 the binding coalition values are
...
Two limits apply. An order-2 share over two players is not comparable to the same quantity
over a fifteen-head circuit, so what carries weight in the binding result is that the order-1
coefficients are exactly zero rather than that the share reaches one. And \citet{benzer1955}
...
the same-unit verdict. The decomposition was derived from EXP16's results afterward and is a
follow-up rather than a registered prediction.

\subsection{Rescue reversibility (EXP17, L38, $n = 200$)}
...
\begin{table}[t]
\centering
\caption{Summary of pre-registered experiments with completed results. Predictions marked with $\dagger$ were overturned by the data; all overturned predictions strengthened rather than weakened the paper's claim. L53 is excluded from the verdict column due to high seed-to-seed variance (EXP1).}
\label{tab:summary}
...
EXP16 & Same mechanism cross-model & Answer transfers, binding differs & Partial \\
EXP17 & Rescue recovers accuracy & 8\% drop, rescue to 1.0 & Partial \\
...
\paragraph{Scope.}
All Llama experiments use one model (Llama-3.1-70B-Instruct, a minor version update from the paper's 3.0). EXP16 extends to Qwen2.5-14B-Instruct, where the answer lookback dependency structure transfers but the binding mechanism operates differently. We use the paper's released SVD bases and masks rather than recomputing them, so our results test \emph{generalization} to novel stimuli rather than \emph{replicability} from scratch. Self-container controls yielded zero viable pairs; the reality-question control yielded 250 pairs under per-framing filtering and is reported in Section~\ref{sec:experiments}.
...
carving returned no usable estimates, and EXP16 finds the answer dependency structure
...
validity, Figure~\ref{fig:framing}). None of that substitutes for baseline separation. M2
asks whether a matched-rank random subspace achieves comparable interchange accuracy, and no
experiment in this record answers it directly. The rescue control bears on it obliquely:
restoring an ablated L38 from a random subspace of matched rank recovers $0.93$ against an
ablated floor of $0.92$ and a true-subspace rescue of $1.0$, so the direction carries
sample-specific content. That is a restoration test rather than an interchange test, and we
record M2 as addressed rather than established.
...
the answer, not that the model requires them. The gate above is a single experiment: a
matched-rank random-subspace comparison at each identified layer. Reaching ``Triangulated''
