...
Eleven experiments across three models and dozens of layer-concept combinations appear to constitute massive convergent evidence. They are one measurement family applied repeatedly. The subspace identification (DCM) uses the same IIA metric as the intervention experiments---it optimizes a binary mask to maximize IIA. No weight analysis identifies which attention heads implement the retrieval; no stability analysis checks whether the subspace is consistent across random seeds; no independent method confirms the subspace's causal role. Quantity of replications within one method does not substitute for evidence from an independent method.
...
The evidence establishes that information flows through layer ranges in ways consistent with a retrieval process. The labels ``pointer,'' ``address,'' and ``dereference'' carry inferential weight beyond what the method can detect---they import computational specificity from programming languages into a finding about residual stream information flow. Strip the metaphor and the gap between evidence and claim becomes visible.
...
All five claims in the paper rest on a single method family (interchange interventions via NNsight). The experiments in Section~\ref{sec:experiments} add independent evidence: held-out basis validation tests whether the subspace claim survives out-of-sample evaluation, task-specificity controls test discriminant validity using a different task distribution, and cross-stage mediation probes causal dependencies invisible to single-site intervention.
...
Contracting the claim to what the evidence licenses yields the following restated claim:
