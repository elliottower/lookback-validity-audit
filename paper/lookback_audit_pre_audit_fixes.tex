...
The paper's strongest evidence is the answer pointer experiment (Figure 4b): swapping the ``pointer'' while preserving the ``payload'' produces a third answer distinct from both the original and counterfactual outputs. This resists trivial explanation and demonstrates decomposable internal structure. Whether this pointer is belief-specific or a task-general retrieval index remains open. The random subspace and non-ToM controls (Section~\ref{sec:experiments}) test this distinction.
