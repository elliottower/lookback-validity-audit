...
RESULTS_DIR = REPO_ROOT / "results" / "question_framing_v2"
...
def main():
    ...
    for framing_key in FRAMINGS:
        ...
        sub_cis = {}
        n = len(pairs)
        for sub_name, iia in sub_iias.items():
            k = int(round(iia * n))
            lo, hi = wilson_ci(k, n)
            sub_cis[sub_name] = {"iia": iia, "k": k, "n": n,
                                 "ci_lower": lo, "ci_upper": hi,
                                 "ci_width": hi - lo}
            print(f"      Wilson CI: [{lo:.4f}, {hi:.4f}] (width={hi-lo:.4f})")
