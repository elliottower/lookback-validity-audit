...
RESULTS_DIR = REPO_ROOT / "results" / "rescue_reversibility_v2"
...
def main():
    ...
    all_subspaces = load_subspace_specs()
    subspaces = {k: v for k, v in all_subspaces.items()
                 if "answer" in v["lookback_type"]}

    print(f"[{ts()}] Rescue reversibility test v2 (I7, answer subspaces only)")
